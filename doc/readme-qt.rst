LionCoin-qt: Qt4 GUI for LionCoin
===============================
...
Alternatively, install Qt Creator and open the `LionCoin-qt.pro` file.

An executable named `LionCoin-qt` will be built.
...
A warning for people using the *static binary* version of LionCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of LionCoin is linked against libdb5.3.
...
installed causes LionCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
